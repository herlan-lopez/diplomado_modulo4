...
async function updateUser(req,res) {
    ...
    try{
        ...
        if (!username || !password) 
            return res.status(404).json({ message: 'Username or pasword are required' });
...
try{
  if (!status) 
       return res.status(404).json({ message: 'status are required' });
...
return res.status(404).json({ message: 'User not found' });
    ...
    if(user.status ===status)
        return res.status(400).json({ message: 'The user is already in the selected status' });
